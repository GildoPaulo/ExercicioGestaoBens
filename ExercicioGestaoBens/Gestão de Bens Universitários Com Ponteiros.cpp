...
struct Item {
    ...
    string tipo; // Aqui guardo o tipo do item, como "propriedade" ou "pessoal" imovel ou movel
...
vector<Item*> lerItens() {
    vector<Item*> itens; // Aqui vou guardar os ponteiros para meus itens
    ifstream arquivo("meus_itens.txt"); // Aqui abro o arquivo para leitura
    if (arquivo.is_open()) {
        Item* item; // Aqui vou criar um ponteiro para um novo item
        while (true) {
            item = new Item(); // Aqui aloco memória para o novo item
            ...
            itens.push_back(item); // Adiciono o ponteiro para o item ao vetor
            if (arquivo.eof()) {
    ...
    return itens; // Retorno o vetor de ponteiros para meus itens
}
...
void escreverItens(const vector<Item*>& itens) {
    ...
    if (arquivo.is_open()) {
        // Percorro o vetor de ponteiros para meus itens
...
void adicionarItem(vector<Item*>& itens) {
    Item* novoItem = new Item(); // Aqui aloco memória para um novo item
    ...
    itens.push_back(novoItem); // Adiciono o ponteiro para o novo item ao vetor
}
...
void excluirItem(vector<Item*>& itens) {
    ...
    for (int i = 0; i < itens.size(); ++i) {
        // Verifico se o nome do item corresponde ao informado pelo usuário
        if (itens[i]->nome == nomeItem) {
            delete itens[i]; // Libero a memória alocada para o item
            itens.erase(itens.begin() + i); // Removo o ponteiro para o objeto Item do vetor
...
int main() {
    ...
    // Libero a memória alocada para os objetos Item
    for (int i = 0; i < itens.size(); ++i) {
        delete itens[i];
    }
